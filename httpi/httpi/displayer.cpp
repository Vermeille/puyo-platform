...
static int not_found_page(const void* /* cls */,
    ...
    MHD_add_response_header(
        response, MHD_HTTP_HEADER_CONTENT_ENCODING, nullptr);
...
static int answer_to_connection(void* cls,
    ...
    std::string method = m;
    ConnInfo* info = static_cast<ConnInfo*>(*con_cls);
    std::cerr << method << " " << url << " upload size: " << *upload_data_size
              << std::endl;
    if (*con_cls == nullptr) {
        info = new ConnInfo;
        *con_cls = info;
    ...
    MHD_get_connection_values(
        connection,
        (MHD_ValueKind)((int)MHD_POSTDATA_KIND | (int)MHD_GET_ARGUMENT_KIND |
                        (int)MHD_RESPONSE_HEADER_KIND | (int)MHD_HEADER_KIND),
    ...
    if (info->page.empty()) {
        ...
        struct MHD_Response* response =
            MHD_create_response_from_buffer(info->page.size(),
                                            (void*)info->page.c_str(),
                                            MHD_RESPMEM_PERSISTENT);
...
void StopHttpInterface() { MHD_stop_daemon(g_daemon); }
